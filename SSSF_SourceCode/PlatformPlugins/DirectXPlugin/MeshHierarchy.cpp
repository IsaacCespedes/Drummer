...
HRESULT CMeshHierarchy::CreateFrame(LPCSTR Name, LPD3DXFRAME *retNewFrame)
{
    ...
    *retNewFrame = newFrame;
	...
	if (Name && strlen(Name))
	{
		...
		newFrame->Name=CUtility::DuplicateCharString(ws.c_str());	
		CUtility::DebugString(L"Added frame: "/*+ToString(Name)+"\n"*/);
	}
	else
	{
		CUtility::DebugString(L"Added frame: no name given\n");
	}
    return S_OK;
}
...
{    
	...
	*retNewMeshContainer = 0;
	...
	if (Name && strlen(Name))
	{
		string s = Name;
		wstring ws(s.begin(), s.end()); 
		newMeshContainer->Name=CUtility::DuplicateCharString(ws.c_str());
		CUtility::DebugString(L"Added mesh: "/*+ToString(Name)+"\n"*/);
	}
	else
	{
		CUtility::DebugString(L"Added Mesh: no name given\n");
	}
	...
	if (numMaterials>0)
	{
		...
		for(DWORD i = 0; i < numMaterials; ++i)
		{
			...
			{
				string s(materials[i].pTextureFilename);
				wstring ws(s.begin(), s.end());
				//wstring texturePath(materials[i].pTextureFilename);
				if (CUtility::FindFile(&ws))
				{
					// Use the D3DX function to load the texture
					if(FAILED(D3DXCreateTextureFromFile(pd3dDevice, ws.c_str(),
						&newMeshContainer->exTextures[i])))
					{
						CUtility::DebugString(L"Could not load texture: "/*+ws+"\n"*/);					
					}
				}
				else
				{
					CUtility::DebugString(L"Could not find texture: "/*+ToString(materials[i].pTextureFilename)+"\n"*/);					
				}
			}
		}
	}
	...
	if (pSkinInfo)
	{
		...
		CUtility::DebugString(L"Mesh has skinning info.\n Number of bones is: "/*+ToString(numBones)+"\n"*/);
	...
	if (effectInstances)
	{
		if (effectInstances->pEffectFilename)
			CUtility::DebugString(L"This .x file references an effect file. Effect files are not handled by this demo\n");
	}
