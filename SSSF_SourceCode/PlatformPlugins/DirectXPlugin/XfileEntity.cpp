...
*/
bool CXFileEntity::Load(const wstring &filename)
{
	...
	wstring currentDirectory=CUtility::GetTheCurrentDirectory();

	wstring xfilePath;
	CUtility::SplitPath(currentDirectory + filename,&xfilePath,&m_filename);

	SetCurrentDirectory(xfilePath.c_str());
	//WCHAR * wc = (currentDirectory + filename).c_str();
	...
	HRESULT hr = D3DXLoadMeshHierarchyFromX(L"C:\\Users\\Isaac\\Documents\\Visual Studio 2010\\Projects\\CSE380_Spring2012_Projects\\Apps\\DrummerBoyGame\\mesh\\drummer.x"
		/*(currentDirectory + filename).c_str()*/, D3DXMESH_MANAGED, m_d3dDevice, 
		memoryAllocator, NULL, &m_frameRoot, &m_animController);

	delete memoryAllocator;
	memoryAllocator=0;

	SetCurrentDirectory((LPCTSTR)currentDirectory.c_str());
